...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("https://www.hymosetups.com/api/v1/forget-password", {
        method: "POST",
        headers: {
      ...
      setError(err instanceof Error ? err.message : t.auth.forgotPassword.error || "Failed to send reset email. Please try again.")
    } finally {
        ...
        <div className="relative z-10 w-full max-w-md">
          <Card className="login-glass-card relative border-0 bg-transparent shadow-none overflow-hidden rounded-2xl backdrop-blur-sm">
            ...
            <CardHeader className="text-center pb-2 items-center">
              ...
              <CardDescription className="text-muted-foreground/90">{t.auth.forgotPassword.subtitle}</CardDescription>
            </CardHeader>
              ...
              <CardContent className="space-y-4 pb-2">
                ...
                {!success && (
                  <div className="space-y-2">
                    <Label htmlFor="forgot-email" className="text-foreground/90">{t.auth.forgotPassword.email}</Label>
                    <Input
                      id="forgot-email"
                      type="email"
                      ...
                      autoComplete="email"
                    />
                    <p className="text-xs text-muted-foreground">{t.auth.forgotPassword.hint}</p>
                  </div>
                )}
              </CardContent>
